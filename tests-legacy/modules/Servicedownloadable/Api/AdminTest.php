...
class AdminTest extends \BBTestCase
{
    ...
    public function testGetDownloadLinksMissingProductId(): void
    {
        ...
        $this->api->get_download_links($data);
    }

    public function testGetDownloadLinksProductNotFound(): void
    {
        $data = ['id' => 999];
    ...
    public function testGetDownloadLinksNoFile(): void
    {
        $data = ['id' => 1];
        $product = new \Model_Product();
        $product->loadBean(new \DummyBean());
        ...
        $result = $this->api->get_download_links($data);
        $this->assertIsArray($result);
        $this->assertEmpty($result);
    }

    public function testGetDownloadLinksSuccess(): void
    {
        $data = ['id' => 1];
        $product = new \Model_Product();
        $product->loadBean(new \DummyBean());
        ...
        $this->di['url']->expects($this->once())
            ->method('adminLink')
            ->with('servicedownloadable/get-file/1')
            ->willReturn('/admin/servicedownloadable/get-file/1');

        $result = $this->api->get_download_links($data);
        
        $this->assertIsArray($result);
        $this->assertCount(1, $result);
        $this->assertEquals('test_file.txt', $result[0]['filename']);
        $this->assertEquals('/admin/servicedownloadable/get-file/1', $result[0]['download_url']);
        $this->assertStringContains(md5('test_file.txt'), $result[0]['direct_path']);
    }
